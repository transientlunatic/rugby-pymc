...
class RugbyModel:
    ...
    def build(self, df: pd.DataFrame, score_type: str = "tries") -> pm.Model:
        ...
        with pm.Model() as model:
            ...
            n_positions = 23  # Rugby union squad size
            ...
            # Team-season effect (coaching, tactics, squad quality)
            ...
            log_lambda = (
                ...
                + theta_position[position - 1]  # Convert 1-indexed to 0-indexed
                + eta_home * is_home
    ...
    def build_joint(self, df: pd.DataFrame) -> pm.Model:
        ...
        with pm.Model() as model:
            ...
            # Player ability (shared across score types, but with type-specific loading)
            beta_player_raw = pm.Normal("beta_player_raw", mu=0, sigma=1, shape=n_players)

            # Team-season effect (shared)
            ...
            # Player effect loadings per score type
            ...
            for s, score_type in enumerate(self.config.score_types):
                ...
                team_season_idx = pm.Data(f"team_season_idx_{score_type}", data["team_season_idx"])
                position = pm.Data(f"position_{score_type}", data["position"])
    ...
    def _prepare_data(self, df: pd.DataFrame, score_type: str) -> dict[str, np.ndarray]:
        ...
        position = df_filtered["position"].values
    ...
    def get_player_rankings(
        ...
        if score_type and f"y_{score_type}" in trace.posterior:
            # Joint model - need to compute effective player effect
            ...
            beta = sigma * lambda_p[..., None] * beta_raw
        else:
